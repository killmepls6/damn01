...
export function useUploadProgress(options: UseUploadProgressOptions) {
  ...
  useEffect(() => {
    ...
    console.log(`[UploadProgress] Subscribing to upload:${uploadId}`);
    ...
    const unsubscribeProgress = subscribe('upload:progress', (payload: UploadProgressData) => {
      if (payload.uploadId === uploadId) {
        console.log(`[UploadProgress] Progress update:`, payload);
        setProgress(payload);
        onProgress?.(payload);
      }
    });
    ...
    const unsubscribeComplete = subscribe('upload:complete', (payload: UploadProgressData) => {
      if (payload.uploadId === uploadId && !callbacksCalledRef.current.complete) {
        console.log(`[UploadProgress] Upload complete:`, payload);
        setProgress(payload);
        setIsComplete(true);
    ...
    const unsubscribeFailed = subscribe('upload:failed', (payload: UploadProgressData) => {
      if (payload.uploadId === uploadId && !callbacksCalledRef.current.error) {
        console.log(`[UploadProgress] Upload failed:`, payload);
        setProgress(payload);
        setHasError(true);
    ...
    return () => {
      console.log(`[UploadProgress] Unsubscribing from upload:${uploadId}`);
      unsubscribeProgress();
      unsubscribeComplete();
      unsubscribeFailed();
    };
  ...
  useEffect(() => {
    ...
    console.log(`[UploadProgress] WebSocket not connected, using HTTP polling fallback for ${uploadId}`);
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        console.error('[UploadProgress] HTTP polling error:', error);
      }
