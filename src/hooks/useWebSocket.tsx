...
export function useWebSocket(
  ...
  const connect = useCallback(() => {
    ...
    if (!shouldConnectRef.current) {
      console.log('[useWebSocket] Connection disabled');
      return;
    }
    ...
    try {
      ...
      ws.onclose = () => {
        ...
        if (reconnect && shouldConnectRef.current && reconnectCountRef.current < reconnectAttempts) {
          ...
          console.log(`[useWebSocket] Reconnecting in ${delay}ms (attempt ${reconnectCountRef.current + 1})`);
          ...
          if (subscribers) {
            subscribers.forEach(callback => {
              try {
                ...
                console.error(`[useWebSocket] Error in subscriber for ${message.type}:`, error);
              }
            });
          }
        } catch (error) {
  ...
  const subscribe = useCallback((eventType: string, callback: (payload: any) => void) => {
    ...
    console.log(`[useWebSocket] Subscribed to ${eventType} (${subscribers.size} subscribers)`);
    ...
    return () => {
      ...
      if (subscribers) {
        ...
        console.log(`[useWebSocket] Unsubscribed from ${eventType} (${subscribers.size} subscribers remaining)`);
