...
export function SEO({
  ...
  const baseUrl = typeof window !== 'undefined' ? window.location.origin : 'https://mangaverse.app';
  const currentUrl = canonicalUrl || `${baseUrl}${location}`;
  
  const fullTitle = title 
    ? `${title} | MangaVerse`
    : "MangaVerse - Discover Amazing Manga & Manhwa";
  
  const defaultDescription = "Explore thousands of manga and manhwa series with ratings, latest updates, and trending recommendations. Your ultimate destination for manga discovery.";
  ...
  useEffect(() => {
    ...
    updateMetaTag('og:site_name', 'MangaVerse', true);
